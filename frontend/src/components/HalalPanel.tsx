...
interface HalalPanelProps {
  ticker: string;
  /** FR-A06: when provided and different from current status, renders a comparison row */
  previousStatus?: HalalStatus;
}
...
export function HalalPanel({ ticker, previousStatus }: HalalPanelProps) {
  ...
  // FR-A06: show comparison row only when previous status differs from current
  const showStatusChange = previousStatus !== undefined && previousStatus !== data.status;
  const prevStyles = previousStatus ? statusStyles[previousStatus] : null;

  // Compute nonHalalPct for PurificationCalculator
  // interest_income_ratio is a decimal (0–1); convert to percent for the calculator
  const nonHalalPct: number | null =
    data.status === 'PurificationRequired'
      ? data.interest_income_ratio != null
        ? data.interest_income_ratio * 100
        : null
  ...
  return (
    ...
    >
      <Card variant="default" className="space-y-4">
        ...
        {/* FR-A06: Status change comparison row */}
        {showStatusChange && prevStyles && previousStatus && (
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div className={`rounded-lg px-3 py-2 border ${prevStyles.bg} ${prevStyles.text} ${prevStyles.border} text-center`}>
              <p className="text-xs opacity-60 mb-0.5">السابق</p>
              <p className="font-semibold">{statusLabel[previousStatus]}</p>
            </div>
            <div className={`rounded-lg px-3 py-2 border ${styles.bg} ${styles.text} ${styles.border} text-center`}>
              <p className="text-xs opacity-60 mb-0.5">الحالي</p>
              <p className="font-semibold">{statusLabel[data.status]}</p>
            </div>
          </div>
        )}
        ...
        {/* T013: Embed PurificationCalculator for PurificationRequired stocks */}
        {data.status === 'PurificationRequired' && (
          <PurificationCalculator nonHalalPct={nonHalalPct} />
        )}
